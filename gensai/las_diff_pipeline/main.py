...
from pipeline.io_las import read_auto, write_las
...
def run(cfg: Config, only_step: str | None = None) -> None:
    ...
    # ─── 1. 読み込み ───
    logging.info("=== Step 1: Read LAS ===")
    pre = read_auto(cfg.resolve_path(cfg.io["pre_las"]), fallback_epsg=input_epsg)
    post = read_auto(cfg.resolve_path(cfg.io["post_las"]), fallback_epsg=input_epsg)
    ...
    # ─── 2. 前処理 ───
    logging.info("=== Step 2: Preprocess ===")
    ...
    # ─── 3. 位置合わせ ───
    logging.info("=== Step 3: Registration ===")
    ...
    # ─── 4. 差分計算 ───
    logging.info("=== Step 4: Difference ===")
    diff_cfg = cfg.difference
    method = diff_cfg.get("method", "m3c2")
    ...
    # ─── 6. 集約 ───
    logging.info("=== Step 6: Aggregation ===")
    ...
    # ─── 7. エクスポート ───
    logging.info("=== Step 7: Export GeoJSON ===")
