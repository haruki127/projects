...
## 1. 変化に影響されないためのマスク処理（Geopandas）

すべての点で位置合わせをしてしまうと、災害などで大きく地形が変わった場所に引っ張られて位置合わせがズレてしまいます。これを防ぐために、変化していないエリアの点だけを絞り込む機能が備わっています。

## 2. 徹底されたエラー回避（動的インポート）

`geopandas` や `open3d` といった、インストールに少し手間がかかる外部ライブラリを、ファイルの先頭ではなく「関数が必要になった瞬間」にインポートしています。これにより、その機能を使わない人やライブラリがない環境でも、ファイル全体がエラーで動かなくなるのを防いでいます。

## 3. 自前実装のバックアップ（Umeyama法）

Open3Dが使えない場合の保険として、`scipy` の `cKDTree` と `np.linalg.svd` を組み合わせた高精度なICPアルゴリズムを自前で再現しています。最大点数を20万点に制限するなどの実用的なパフォーマンス対策も施されています。

---

# 1文でまとめるなら

`registration.py` は、環境に合わせて外部ライブラリと自前計算を自動で切り替え、変化のない安定領域を基準にして2つの点群の位置を正確に合わせるためのコードです。
