...
災害前後の点群を比較し、「どこがどれだけ高さが変わったか」を計算するモジュールです。主にDSM差分法を使い、オプションでM3C2法も呼べます。
...
バラバラな3D点群を、指定した解像度の格子状グリッド（DSM）に変換します。
...
前後の点群を同じグリッド座標でDSM化し、セルごとに `post - pre` を計算します。
...
**戻り値**：`(diff_raster, bbox)` のタプル。そのまま `write_geotiff()` に渡せます。
...
**補足**：前後の点群の範囲が異なっていても、両者を包む共通のバウンディングボックスで統一するため、同じグリッド座標で差し引きできます。

---

## `write_geotiff(raster, bbox, path, epsg)`

差分ラスタを地理参照情報付きの**GeoTIFF**として書き出します。QGISやArcGISで直接開いて可視化できます。
...
> **依存ライブラリ**：`rasterio` が必要です。未インストールの場合は実行時エラーになります。

---

## `m3c2(pre, post, work_dir, ...)` ※未実装

DSM差分より精度の高い**M3C2法**（点群を直接比較する手法）の予約関数です。現在は `NotImplementedError` を投げます。

利用するには `pdal` のインストールとパイプラインJSONの実装が必要です。**現時点では `method='dsm'` を使用してください。**
